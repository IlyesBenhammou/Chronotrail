...
// Endpoint pour récupérer les coureurs
app.get('/api/coureurs', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        preinscriptioncoureur.nomcoureur AS nom,
        preinscriptioncoureur.prenomcoureur AS prenom,
        EXTRACT(YEAR FROM AGE(preinscriptioncoureur.datenaissance)) AS age,
        preinscriptioncoureur.sexe AS sexe,
        courses.nom AS circuit
      FROM
        preinscriptioncoureur
      JOIN
        courses ON preinscriptioncoureur.id_course = courses.id_course;
    `);
...
// Récupérer les coureurs et les afficher dans un tableau
async function fetchCoureurs() {
    try {
      const response = await fetch('/api/coureurs');
      const coureurs = await response.json();
  
      const tableBody = document.getElementById('coureurTableBody');
  ...
  document.addEventListener('DOMContentLoaded', fetchCoureurs);
