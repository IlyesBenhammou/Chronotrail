...
app.get('/api/coureurs/:id/courses', async (req, res) => {
    ...
    try {
        ...
        console.error('Erreur récupération courses:', err); // 🔧 log utile
        res.status(500).json({ error: 'Erreur serveur' });
    }
});
...
app.get('/api/dossards/:iddossard', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ message: 'Dossard non trouvé' });
...
app.post('/api/assign-dossard', async (req, res) => {
    ...
    try {
        // Libérer l'ancien dossard du coureur (s'il en avait un)
        await pool.query('UPDATE dossards SET idcoureur = NULL, disponible = true WHERE idcoureur = $1', [coureurId]);

        // Assigner le nouveau dossard
        await pool.query('UPDATE dossards SET idcoureur = $1, disponible = false WHERE iddossard = $2', [coureurId, dossardId]);
...
app.post('/api/inscriptions', async (req, res) => {
    ...
    try {
        const exist = await pool.query('SELECT * FROM inscription WHERE idcoureur = $1 AND idcourse = $2', [idcoureur, idcourse]);
        if (exist.rows.length > 0) return res.status(400).json({ message: 'Déjà inscrit.' });
        await pool.query('INSERT INTO inscription (idcoureur, idcourse, heure_depart) VALUES ($1, $2, NOW())', [idcoureur, idcourse]);
        res.json({ message: 'Inscription réussie' });
    } catch (err) {
        res.status(500).json({ error: 'Erreur serveur' });
    }
});
// === Route pour mettre à jour la disponibilité d’un dossard ===
